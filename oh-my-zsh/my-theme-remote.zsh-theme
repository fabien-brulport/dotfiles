...
PROMPT='╭─%B$(virtualenv_prompt_info)%b%{$fg[green]%}%n@%{$fg[green]%}%m%{$reset_color%} %(!.%{$fg[red]%}.%{$fg[green]%})%~$reset_color$(git_prompt_info)$(git_prompt_status)$reset_color$(git_commits_ahead)$(git_commits_behind)
╰─ %B${user_symbol}%b '
