# Generated by Powerlevel10k configuration wizard on 2022-06-02 at 16:08 CEST.
...
# 2 lines, disconnected, left frame, blue-ornaments, sparse, few icons, fluent,
...
() {
  ...
  function my_git_formatter() {
    ...
    if (( $1 )); then
      ...
      local   c_branch='%5F'  # violet foreground
      ...
      local       meta='%f'  # default foreground
      local   c_branch='%f'  # default foreground
    ...
    if [[ -n $VCS_STATUS_LOCAL_BRANCH ]]; then
      ...
      res+="${c_branch}${(g::)POWERLEVEL9K_VCS_BRANCH_ICON}${branch//\%/%%}"
    fi
    ...
    if [[ -n $VCS_STATUS_TAG
      ...
      res+="${meta}#${c_branch}${tag//\%/%%}"
    fi

    # Display the current Git commit if there is no branch and no tag.
    # Tip: To always display the current Git commit, delete the next line.
    [[ -z $VCS_STATUS_LOCAL_BRANCH && -z $VCS_STATUS_TAG ]] &&  # <-- this line
      res+="${meta}@${c_branch}${VCS_STATUS_COMMIT[1,8]}"

    # Show tracking branch name if it differs from local branch.
    if [[ -n ${VCS_STATUS_REMOTE_BRANCH:#$VCS_STATUS_LOCAL_BRANCH} ]]; then
      res+="${meta}:${c_branch}${(V)VCS_STATUS_REMOTE_BRANCH//\%/%%}"
    fi
